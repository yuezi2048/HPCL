...
class TT3(GeneralRecommender):
    def __init__(self, config, dataset):
        super(TT3, self).__init__(config, dataset)
        ...
        community_map_path = os.path.join(dataset_path, 'item_to_fused_community.pt')
    ...
    # =================================================================
        ...
        # --- 2. 计算 g(u, j) - 负样本被“误点”的概率 ---
        with torch.no_grad():
            # 计算用户在视觉和文本模态上对负样本的偏好分数
            score_neg_v = (v_rep_users * v_rep_neg_items).sum(dim=-1)
            score_neg_t = (t_rep_users * t_rep_neg_items).sum(dim=-1)

            scores_neg_modal = torch.stack([score_neg_v, score_neg_t], dim=1)

            # 找到用户在单一模态上对负样本的最大偏好
            max_pref_neg, _ = torch.max(scores_neg_modal, dim=1)

            # 仿照DA-MRS论文公式(7)，计算“误点”概率 g(u, j)
            # gamma 是可调超参数，这里简化为1.0
            gamma = 1.0
            g_uj = torch.pow(torch.sigmoid(max_pref_neg), gamma)

        # --- 3. 构建新的 D-BPR 损失函数 ---
        # sigmoid(score_diff) 表示 P(i >_u j | theta)
        prob_i_gt_j = torch.sigmoid(pos_scores - neg_scores)

        # 仿照DA-MRS论文公式(8)，整个事件的概率是 P(obs) = P(obs|true)P(true) + P(obs|false)P(false)
        # P(obs) = f(u,i) * P(i >_u j) + g(u,j) * P(j >_u i)
        # P(j >_u i) = 1 - P(i >_u j)
        prob_event = f_ui * prob_i_gt_j + g_uj * (1 - prob_i_gt_j)

        # 取对数似然并求负均值
        loss = -torch.mean(torch.log(prob_event + 1e-8))  # 加epsilon防止log(0)

        return loss


    def _calculate_reg_loss(self, user):
        """ 计算L2正则化损失 """
    ...
    def _calculate_mask_losses(self):
        """ 计算两种掩码/对比损失 (mask_f_loss 和 mask_g_loss) """
        # mask_f_loss: 特征稳定性损失
        with torch.no_grad():
        ...
        return mask_f_loss + mask_g_loss

    def _calculate_disentanglement_loss(self, pos_items, neg_items):
        """ 计算完整的引导式解耦损失 """
    ...
    def calculate_loss(self, interaction):
        ...
        # ===============================================================
        # ====== 新增断言，用于捕获错误的正样本索引 START ================
        # ===============================================================
        assert torch.all(pos_items >= 0) and torch.all(pos_items < self.n_items), \
            f"越界错误！发现无效的正样本物品索引。最大值: {pos_items.max()}, 最小值: {pos_items.min()}"
        # ===============================================================
        # ====== 新增断言 END =============================================
        # ===============================================================

        # 2. 计算各个独立的损失模块
        loss_bpr = self._calculate_bpr_loss(user, pos_items, neg_items, pos_scores, neg_scores)

        loss_reg = self._calculate_reg_loss(user)
        loss_mask = self._calculate_mask_losses()
        loss_disentangle = self._calculate_disentanglement_loss(pos_items, neg_items)

        # 3. 加总所有损失
        total_loss = loss_bpr + loss_reg + loss_mask + loss_disentangle
