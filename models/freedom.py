...
class FREEDOM(GeneralRecommender):
    """
       ...
       FREEDOM模型继承自Model类，本初始化方法主要负责设置模型参数、
       初始化嵌入层、加载数据集信息、构建邻接矩阵等。
       """
    ...
    def pre_epoch_processing(self):
        ...
        # 根据前面计算得到的保留边索引(degree_idx)，从原始图边(edge_indices)中选取对应的边
        keep_indices = self.edge_indices[:, degree_idx]

        # 对选中的边进行归一化处理，用于后续构建邻接矩阵的值
        keep_values = self._normalize_adj_m(keep_indices, torch.Size((self.n_users, self.n_items)))

        # 将归一化后的值拼接两次，因为邻接矩阵是无向的，需要同时表示正向和反向的连接
        all_values = torch.cat((keep_values, keep_values))

        # 为了构建用户-物品二部图的完整邻接矩阵，将物品节点的索引加上用户的总数(self.n_users)
        # 这样用户和物品节点在同一个图中有唯一的索引标识
        keep_indices[1] += self.n_users

        # 拼接原始边和它们的转置（即交换起点和终点），以形成对称的邻接矩阵
        all_indices = torch.cat((keep_indices, torch.flip(keep_indices, [0])), 1)

        # 使用稀疏张量构造最终的掩码邻接矩阵，并将其移动到指定设备（如GPU）
        # 这个邻接矩阵会在训练过程中作为图神经网络的消息传递结构
