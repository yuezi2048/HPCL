...
class TT6(GeneralRecommender):
    def __init__(self, config, dataset):
        super(TT6, self).__init__(config, dataset)
        ...
        # --- ↓↓↓ 添加下面的所有代码 ↓↓↓ ---
        max_neighbors = max(len(v[0]) for v in self.user_graph_dict.values()) if self.user_graph_dict else 0
        print(f"用户最大邻居数: {max_neighbors}")

        # 创建用于存储邻居节点和权重的填充张量
        neighbor_nodes_tensor = torch.full((self.n_users, max_neighbors), -1, dtype=torch.long)
        neighbor_weights_tensor = torch.full((self.n_users, max_neighbors), 0.0, dtype=torch.float)
        # 记录每个用户的真实邻居数量
        neighbor_counts = torch.zeros(self.n_users, dtype=torch.long)

        for user_id, (nodes, weights) in self.user_graph_dict.items():
            num_neighbors = len(nodes)
            if num_neighbors > 0:
                neighbor_nodes_tensor[user_id, :num_neighbors] = torch.tensor(nodes, dtype=torch.long)
                neighbor_weights_tensor[user_id, :num_neighbors] = torch.tensor(weights, dtype=torch.float)
                neighbor_counts[user_id] = num_neighbors

        # 将这些张量注册为模型的缓冲区，并移动到GPU
        self.register_buffer('neighbor_nodes', neighbor_nodes_tensor)
        self.register_buffer('neighbor_weights', neighbor_weights_tensor)
        self.register_buffer('neighbor_counts', neighbor_counts)

        # 删除CPU上的字典，释放内存
        del self.user_graph_dict
        # --- ↑↑↑ 添加上面的所有代码 ↑↑↑ ---
    ...
    # --- ↓↓↓ 用这个函数替换掉旧的 topk_sample ↓↓↓ ---
    def topk_sample(self, k):
        """
        (GPU优化版) 为所有用户一次性采样固定k个邻居及其权重。
        - 核心逻辑: 完全使用PyTorch张量操作，在GPU上完成。
        """
        # 1. 获取在GPU上的邻居数据
        n_users = self.n_users
        device = self.device
        max_neighbors = self.neighbor_nodes.size(1)

        # 2. 准备输出张量
        sampled_nodes = torch.zeros((n_users, k), dtype=torch.long, device=device)
        sampled_weights = torch.zeros((n_users, k), dtype=torch.float, device=device)

        # 3. 处理邻居数 >= k 的情况 (直接截取)
        enough_neighbors_mask = (self.neighbor_counts >= k)
        if enough_neighbors_mask.any():
            sampled_nodes[enough_neighbors_mask] = self.neighbor_nodes[enough_neighbors_mask, :k]
            sampled_weights[enough_neighbors_mask] = self.neighbor_weights[enough_neighbors_mask, :k]

        # 4. 处理 0 < 邻居数 < k 的情况 (需要填充)
        pad_neighbors_mask = (self.neighbor_counts > 0) & (self.neighbor_counts < k)
        if pad_neighbors_mask.any():
            users_to_pad = torch.where(pad_neighbors_mask)[0]

            for user_id in users_to_pad:
                num_original = self.neighbor_counts[user_id].item()
                num_to_pad = k - num_original

                # 从已有邻居中随机采样以进行填充
                padding_indices = torch.randint(0, num_original, (num_to_pad,), device=device)

                original_nodes = self.neighbor_nodes[user_id, :num_original]
                original_weights = self.neighbor_weights[user_id, :num_original]

                padded_nodes = original_nodes[padding_indices]
                padded_weights = original_weights[padding_indices]

                sampled_nodes[user_id] = torch.cat([original_nodes, padded_nodes])
                sampled_weights[user_id] = torch.cat([original_weights, padded_weights])

        # 5. 对权重进行 Softmax 归一化
        sampled_weights = F.softmax(sampled_weights, dim=1)

        # 6. 返回最终结果 (注意格式匹配 pre_epoch_processing 的要求)
        return sampled_nodes.tolist(), sampled_weights
    # --- ↑↑↑ 替换结束 ↑↑↑ ---