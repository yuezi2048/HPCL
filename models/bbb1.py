...
class BBB1(GeneralRecommender):
    def __init__(self, config, dataset):
        super(BBB1, self).__init__(config, dataset)
        ...
        # (新) 为 G 和 S 特征创建 GCN 通道
        # 注意：G 和 S 特征的维度是 embedding_dim (64)，而不是原始特征维度
        # 但 GCN 类内部的 MLP 会处理维度转换，所以初始化时仍可传入 v_feat/t_feat
        # 我们将在 forward 阶段传入真正的 G/S 特征

        # 创建一个具有正确维度 (embedding_dim = 64) 的占位符特征，用于 GCN 初始化
        dummy_feat = torch.zeros(self.n_items, self.feat_embed_dim).to(self.device)

        if self.v_feat is not None:
            self.v_gcn_g = GCN(self.dataset, batch_size, num_user, num_item, dim_x, self.aggr_mode, dim_latent=64,
                               device=self.device, features=dummy_feat)
            self.v_gcn_s = GCN(self.dataset, batch_size, num_user, num_item, dim_x, self.aggr_mode, dim_latent=64,
                               device=self.device, features=dummy_feat)
        if self.t_feat is not None:
            self.t_gcn_g = GCN(self.dataset, batch_size, num_user, num_item, dim_x, self.aggr_mode, dim_latent=64,
                               device=self.device, features=dummy_feat)
            self.t_gcn_s = GCN(self.dataset, batch_size, num_user, num_item, dim_x, self.aggr_mode, dim_latent=64,
                               device=self.device, features=dummy_feat)

        # (新) G 和 S 融合层
        self.fusion_layer_v = nn.Linear(self.dim_latent * 2, self.dim_latent)
        self.fusion_layer_t = nn.Linear(self.dim_latent * 2, self.dim_latent)
    ...
    def forward(self, interaction):
        ...
        # (新) 1. 首先运行 mge 获取 G/S 特征
        item_image_g, item_text_g, item_image_s, item_text_s = self.mge()

        # (新) 2. G 特征通道传播
        v_rep_g, v_preference_g = self.v_gcn_g(self.edge_index_dropv, self.edge_index, item_image_g)
        t_rep_g, t_preference_g = self.t_gcn_g(self.edge_index_dropt, self.edge_index, item_text_g)

        # (新) 3. S 特征通道传播
        v_rep_s, v_preference_s = self.v_gcn_s(self.edge_index_dropv, self.edge_index, item_image_s)
        t_rep_s, t_preference_s = self.t_gcn_s(self.edge_index_dropt, self.edge_index, item_text_s)

        # (新) 4. 融合 G 和 S 通道的输出
        # 这里使用拼接 + MLP 的方式融合
        self.v_rep = F.leaky_relu(self.fusion_layer_v(torch.cat([v_rep_g, v_rep_s], dim=1)))
        self.t_rep = F.leaky_relu(self.fusion_layer_t(torch.cat([t_rep_g, t_rep_s], dim=1)))

        # 保留 preference 用于计算 reg_loss
        self.v_preference = v_preference_g
        self.t_preference = t_preference_g
        ...
        # (修改) 噪声路径也必须遵循多通道架构
        # 1. G 特征通道的噪声传播
        v_rep_g_n1, _ = self.v_gcn_g(self.edge_index_dropv, self.edge_index, item_image_g, perturbed=True)
        t_rep_g_n1, _ = self.t_gcn_g(self.edge_index_dropt, self.edge_index, item_text_g, perturbed=True)
        v_rep_g_n2, _ = self.v_gcn_g(self.edge_index_dropv, self.edge_index, item_image_g, perturbed=True)
        t_rep_g_n2, _ = self.t_gcn_g(self.edge_index_dropt, self.edge_index, item_text_g, perturbed=True)

        # 2. S 特征通道的噪声传播
        v_rep_s_n1, _ = self.v_gcn_s(self.edge_index_dropv, self.edge_index, item_image_s, perturbed=True)
        t_rep_s_n1, _ = self.t_gcn_s(self.edge_index_dropt, self.edge_index, item_text_s, perturbed=True)
        v_rep_s_n2, _ = self.v_gcn_s(self.edge_index_dropv, self.edge_index, item_image_s, perturbed=True)
        t_rep_s_n2, _ = self.t_gcn_s(self.edge_index_dropt, self.edge_index, item_text_s, perturbed=True)

        # 3. 融合得到噪声视角 1 的 v_rep 和 t_rep
        self.v_rep_n1 = F.leaky_relu(self.fusion_layer_v(torch.cat([v_rep_g_n1, v_rep_s_n1], dim=1)))
        self.t_rep_n1 = F.leaky_relu(self.fusion_layer_t(torch.cat([t_rep_g_n1, t_rep_s_n1], dim=1)))

        # 4. 融合得到噪声视角 2 的 v_rep 和 t_rep
        self.v_rep_n2 = F.leaky_relu(self.fusion_layer_v(torch.cat([v_rep_g_n2, v_rep_s_n2], dim=1)))
        self.t_rep_n2 = F.leaky_relu(self.fusion_layer_t(torch.cat([t_rep_g_n2, t_rep_s_n2], dim=1)))
